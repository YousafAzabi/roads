...
loop = (input, callback) => {
  ...
  callback(mismatch, arrayOS, arrayOSM, roadCounter);
}
...
exports.start = (input, output, time = new Date()) => {
  loop(input, (mismatch, OS, OSM, roadCounter) => { //call main function loop
    console.log('Writing data to files');
    //write data to files
    io.write(output[0], OS);
    io.write(output[1], OSM);
    io.write(output[2], mismatch);
    print.report(roadCounter); //print report of number of matches of links
